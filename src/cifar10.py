...
#DATA_URL = 'http://www.cs.toronto.edu/~kriz/cifar-10-pyary.tar.gz'
...
def _variable_with_weight_decay(name, shape, stddev, wd):
  ...
  var = _variable_on_cpu(
      name,
      shape,
      tf.truncated_normal_initializer(stddev=stddev, dtype=dtype, seed=0))
  if wd is not None:
...
def loss(logits, labels, scope_name):
  ...
  # decay terms (L2 loss).
  return tf.add_n(tf.get_collection('losses_'+scope_name, scope=scope_name), name='total_loss_'+scope_name)
...
def train(total_loss, scope_name):
  ...
  return apply_gradient_op, grads, opt
